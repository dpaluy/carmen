...
module Carmen
  class Continent
    ...
    def name
      Carmen.i18n_backend.translate("continents.#{code}.name")
    end
    ...
    def countries
      if sub_continents.any?
        sub_continents.map(&:countries).flatten
      else
    ...
    def self.all
      continents.reject(&:world?)
    end

    def self.world
      continents.select(&:world?).first
    end
    ...
    class << self
      ...
      def load_territory_containment
